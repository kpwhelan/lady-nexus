    ...
    <div className='mx-10'>
        ...
        {comments.reverse().map(comment => (
            <div key={comment.id} className='bg-sage/10 m-3 p-3 rounded-lg'>
                ...
                <Comment posts={posts} post_id={post_id} currentUser={currentUser} comment={comment} updatePosts={updatePosts} updatePostsForMyPosts={updatePostsForMyPosts} post_id={post_id} toggleSetModalOpen={toggleSetModalOpen} deleteCommentError={deleteCommentError} commentIdToDelete={commentIdToDelete} />
            </div>
        ))}
    </div>
  )
